...
describe("Cares.Sol", function () {
    async function deployCares() {
        ...
        const caresContract = await cares.deploy(
            owner,
    ...
    describe("Deployment", function () {
        it("Should deploy correctly", async function () {
            const { caresContract, owner, tokenContract } = await loadFixture(
                deployCares
            );
            ...
            expect(await caresContract.BEZY_TOKEN_CONTRACT()).to.equal(
                tokenAddress
            );
        });
    });
    ...
    describe("Adding Campaign", function () {
        it("Should add new Campaign, Emit Event and Assert Campaign Information", async function () {
            const { owner, caresContract, otherAccount } = await loadFixture(
                deployCares
            );
            ...
            // Revert
            await expect(
                caresContract.addCampaign(uri, 0, otherAccount)
            ).to.be.revertedWithCustomError(caresContract, "Hopes__NoGoalSet");

            // Event Emission
            await expect(
                caresContract.addCampaign(uri, goal, otherAccount)
        ...
        it("Should revert when adding zero goal amount to new Campaign", async function () {
            const { owner, caresContract, otherAccount } = await loadFixture(
                deployCares
            );
    ...
    describe("Funding Campaign", function () {
        it("Should fund Campaign, Emit Event, Assert funding amount", async function () {
            const { caresContract, owner, tokenContract, otherAccount } =
                await loadFixture(deployCares);
            ...
            // Revert
            await expect(
                caresContract.fundCampaign(0, 0)
            ).to.be.revertedWithCustomError(
                caresContract,
                "Hopes__InsufficientFunds"
            );
        ...
        it("Should revert campaign funding with zero amount", async function () {
            const { caresContract, owner, tokenContract, otherAccount } =
                await loadFixture(deployCares);
    ...
    describe("Changing Token", function () {
        it("Should assert token address change", async function () {
            const { caresContract, owner, tokenContract, otherAccount } =
                await loadFixture(deployCares);
            ...
            await expect(
                caresContract
                    ...
                    .updateTokenContract(newTokenAddress)
            ).to.be.revertedWithCustomError(
                caresContract,
                "Hopes__Unauthorized"
            );
        ...
        it("Should revert if not owner changing token address", async function () {
            const { caresContract, owner, tokenContract, otherAccount } =
                await loadFixture(deployCares);
